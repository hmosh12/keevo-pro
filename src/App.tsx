...
import { auth, db, googleProvider, signInWithPopup, handleFirestoreError } from './firebase';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Sync Company Info
    const unsubscribeCompany = onSnapshot(doc(db, 'companies', user.companyId), async (docSnap) => {
      if (docSnap.exists()) {
        ...
        try {
          ...
          handleFirestoreError(err, 'create', `companies/${user.companyId}`);
        }
      }
    });
