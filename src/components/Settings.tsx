...
export default function Settings({ 
  ...
  const [paymentMethod, setPaymentMethod] = useState<'mastercard' | 'zaincash' | 'cash' | null>(null);
  ...
  const sections = [
    ...
    { id: 'subscription', label: isRTL ? 'الاشتراك' : 'Subscription', icon: ShieldCheck },
  ...
  const handleAddStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || (isRTL ? 'حدث خطأ أثناء إضافة الموظف' : 'Error adding staff'));
      }
    } catch (error) {
    ...
    <div className="flex flex-col md:flex-row gap-8">
      ...
      <div className="flex-1 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        {activeSection === 'subscription' && (
          <div className="p-8">
            <div className="mb-8">
              <h3 className="text-2xl font-bold text-slate-800">{isRTL ? 'حالة الاشتراك' : 'Subscription Status'}</h3>
              <p className="text-slate-500 text-sm mt-1">
                {isRTL ? 'إدارة اشتراكك وتفعيل النسخة الكاملة من KEEVO' : 'Manage your subscription and activate KEEVO full version'}
              </p>
            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="p-8 bg-white border border-slate-100 rounded-[32px] shadow-sm relative overflow-hidden">
                    <div className={cn(
                      ...
                      companyInfo.isSubscribed ? "bg-emerald-500" : "bg-amber-500"
                    )} />
                    
                    <div className="flex flex-col sm:flex-row items-center gap-6 relative z-10">
                      <div className={cn(
                        "w-20 h-20 rounded-3xl flex items-center justify-center shadow-lg",
                        companyInfo.isSubscribed ? "bg-emerald-50 text-emerald-600 shadow-emerald-50" : "bg-amber-50 text-amber-600 shadow-amber-50"
                      )}>
                        <ShieldCheck size={40} />
                      </div>
                      <div className="text-center sm:text-left rtl:sm:text-right">
                        <div className="flex items-center justify-center sm:justify-start gap-2 mb-2">
                          <h4 className="text-xl font-bold text-slate-800">
                            {companyInfo.isSubscribed ? (isRTL ? 'خطة احترافية نشطة' : 'Active Pro Plan') : (isRTL ? 'الفترة التجريبية' : 'Free Trial Period')}
                          </h4>
                          <span className={cn(
                            "px-2 py-0.5 rounded-full text-[10px] uppercase font-bold tracking-wider",
                            companyInfo.isSubscribed ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                          )}>
                            {isRTL ? (companyInfo.isSubscribed ? 'نشط' : 'تجريبي') : (companyInfo.isSubscribed ? 'Active' : 'Trial')}
                          </span>
                        </div>
                        <p className="text-slate-500 text-sm">
                          {companyInfo.isSubscribed 
                            ? (isRTL ? 'أنت تستمتع بكافة ميزات KEEVO دون قيود.' : 'You are enjoying all KEEVO features without limits.')
                            : (isRTL ? 'لديك وصول كامل للميزات لمدة شهر و3 أيام من تاريخ البدء.' : 'You have full access to features for 1 month and 3 days from start date.')}
                        </p>
                      </div>
                    </div>
                    ...
                    {!companyInfo.isSubscribed && (
                      <div className="mt-12 space-y-8">
                        {/* Plan Selection */}
                        <div>
                          <h5 className="text-sm font-bold text-slate-800 mb-4 uppercase tracking-wider">
                            {isRTL ? 'اختر الخطة المناسبة' : 'Select a Plan'}
                          </h5>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <button 
                              onClick={() => setSelectedPlan('monthly')}
                              className={cn(
                                "p-6 rounded-3xl border-2 transition-all text-left rtl:text-right relative overflow-hidden",
                                selectedPlan === 'monthly' ? "border-indigo-600 bg-indigo-50/30" : "border-slate-100 bg-white hover:border-slate-200"
                              )}
                            >
                              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">{isRTL ? 'شهري' : 'MONTHLY'}</p>
                              <div className="flex items-baseline gap-1">
                                <span className="text-3xl font-black text-slate-800">$29</span>
                                <span className="text-sm text-slate-500 font-medium">/{isRTL ? 'شهر' : 'mo'}</span>
                              </div>
                              {selectedPlan === 'monthly' && <CheckCircle2 className="absolute top-4 right-4 rtl:left-4 rtl:right-auto text-indigo-600" size={20} />}
                            </button>
                            <button 
                              onClick={() => setSelectedPlan('yearly')}
                              className={cn(
                                "p-6 rounded-3xl border-2 transition-all text-left rtl:text-right relative overflow-hidden",
                                selectedPlan === 'yearly' ? "border-indigo-600 bg-indigo-50/30" : "border-slate-100 bg-white hover:border-slate-200"
                              )}
                            >
                              <div className="absolute top-0 right-0 bg-indigo-600 text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl tracking-widest">
                                {isRTL ? 'وفر 20%' : 'SAVE 20%'}
                              </div>
                              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">{isRTL ? 'سنوي' : 'YEARLY'}</p>
                              <div className="flex items-baseline gap-1">
                                <span className="text-3xl font-black text-slate-800">$279</span>
                                <span className="text-sm text-slate-500 font-medium">/{isRTL ? 'سنة' : 'yr'}</span>
                              </div>
                              {selectedPlan === 'yearly' && <CheckCircle2 className="absolute top-4 right-4 rtl:left-4 rtl:right-auto text-indigo-600" size={20} />}
                            </button>
                          </div>
                        </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <button 
                              ...
                              className={cn(
                                ...
                                paymentMethod === 'mastercard' ? "border-indigo-600 bg-indigo-50/30 text-indigo-700" : "border-slate-100 bg-white hover:border-slate-200 text-slate-500"
                              )}
                            >
                              <div className={cn(
                                "w-12 h-12 rounded-2xl flex items-center justify-center transition-colors shadow-sm",
                                paymentMethod === 'mastercard' ? "bg-indigo-600 text-white" : "bg-slate-50 group-hover:bg-slate-100"
                              )}>
                                <CreditCard size={24} />
                              </div>
                              <span className="text-sm font-bold">{isRTL ? 'ماستر كارد' : 'MasterCard'}</span>
                            </button>
                            ...
                            <button 
                              ...
                              className={cn(
                                "flex flex-col items-center gap-3 p-5 rounded-3xl border-2 transition-all group",
                                paymentMethod === 'zaincash' ? "border-emerald-600 bg-emerald-50/30 text-emerald-700" : "border-slate-100 bg-white hover:border-slate-200 text-slate-500"
                              )}
                            >
                              <div className={cn(
                                "w-12 h-12 rounded-2xl flex items-center justify-center transition-colors shadow-sm",
                                paymentMethod === 'zaincash' ? "bg-emerald-600 text-white" : "bg-slate-50 group-hover:bg-slate-100"
                              )}>
                                <Smartphone size={24} />
                              </div>
                              <span className="text-sm font-bold">{isRTL ? 'زين كاش' : 'Zain Cash'}</span>
                            </button>
                            ...
                            <button 
                              ...
                              className={cn(
                                "flex flex-col items-center gap-3 p-5 rounded-3xl border-2 transition-all group",
                                paymentMethod === 'cash' ? "border-amber-600 bg-amber-50/30 text-amber-700" : "border-slate-100 bg-white hover:border-slate-200 text-slate-500"
                              )}
                            >
                              <div className={cn(
                                "w-12 h-12 rounded-2xl flex items-center justify-center transition-colors shadow-sm",
                                paymentMethod === 'cash' ? "bg-amber-600 text-white" : "bg-slate-50 group-hover:bg-slate-100"
                              )}>
                                <Banknote size={24} />
                              </div>
                              <span className="text-sm font-bold">{isRTL ? 'نقدي' : 'نقدي / تحويل'}</span>
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="pt-4">
                          <button 
                            ...
                            onClick={async () => {
                              if (!user?.companyId || !paymentMethod) return;
                            ...
                            className="w-full py-5 bg-indigo-600 text-white rounded-[32px] font-black text-lg shadow-2xl shadow-indigo-200 hover:bg-indigo-700 hover:-translate-y-1 transition-all disabled:opacity-50 disabled:translate-y-0 disabled:shadow-none flex items-center justify-center gap-3"
                          >
                            {isProcessingPayment ? (
                              ...
                              <>
                                ...
                                <span>
                                  {isRTL 
                                    ? `تفعيل الاشتراك (${selectedPlan === 'monthly' ? '$29' : '$279'})` 
                                    : `Activate Subscription (${selectedPlan === 'monthly' ? '$29' : '$279'})`}
                                </span>
                              </>
                            )}
                          </button>
                          <p className="text-center mt-6 text-xs text-slate-400 font-medium">
                            <Lock size={12} className="inline mr-1 mb-1" />
                            {isRTL 
                              ? 'عملية دفع آمنة ومشفرة 256-بت. تواصل مع الدعم الفني لأي استفسار.' 
                              : 'Secure 256-bit encrypted payment. Contact support for any assistance.'}
                          </p>
                        </div>
                      </div>
                    )}
                  ...
                  {companyInfo.isSubscribed && (
                    <div className="p-6 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-center gap-4">
                      ...
                      <p className="text-sm text-emerald-800 font-medium">
                        {isRTL 
                          ? 'شكراً لثقتكم باختيار KEEVO. اشتراككم نشط ولا يتطلب أي إجراء إضافي.' 
                          : 'Thank you for choosing KEEVO. your subscription is active and requires no further action.'}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="p-6 bg-slate-50 border border-slate-100 rounded-[24px]">
                    <h5 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                       ...
                       {isRTL ? 'ماذا تشمل النسخة الاحترافية؟' : 'What is included?'}
                    </h5>
                    <ul className="space-y-3">
                      {[
                        isRTL ? 'دعم فني على مدار الساعة' : '24/7 Technical Support',
                        isRTL ? 'تحديثات تلقائية مجانية' : 'Free automatic updates',
                        isRTL ? 'عدد غير محدود من الفواتير والمنتجات' : 'Unlimited invoices & products',
                        isRTL ? 'تصدير التقارير المالية المتقدمة' : 'Advanced financial reporting',
                        isRTL ? 'عدد غير محدود من الموظفين' : 'Unlimited staff accounts'
                      ].map((item, i) => (
